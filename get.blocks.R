get.blocks <- function (occ, bg.coords) 
{
    ...
    bckg <- rownames(bggrp1)
    bggrp2 <- bg.coords[bg.coords[, 2] < horz & bg.coords[, 1] >= bvert & !(rownames(bg.coords) %in% bckg), ]
    bckg <- c(bckg, rownames(bggrp2))
    bggrp3 <- bg.coords[bg.coords[, 2] > horz & bg.coords[, 1] <= tvert & !(rownames(bg.coords) %in% bckg), ]
    bckg <- c(bckg, rownames(bggrp3))
    bggrp4 <- bg.coords[bg.coords[, 2] >= horz & bg.coords[, 1] > tvert & !(rownames(bg.coords) %in% bckg), ]
    r <- data.frame()
    if (nrow(grp1) > 0) 
        grp1$grp <- 1
    r <- rbind(r, grp1)
    ...
    if (nrow(bggrp1) > 0) 
        bggrp1$grp <- 1
    bgr <- rbind(bgr, bggrp1)
    if (nrow(bggrp2) > 0) 
        bggrp2$grp <- 2
    bgr <- rbind(bgr, bggrp2)
    if (nrow(bggrp3) > 0) 
        bggrp3$grp <- 3
    bgr <- rbind(bgr, bggrp3)
    if (nrow(bggrp4) > 0) 
        bggrp4$grp <- 4
    bgr <- rbind(bgr, bggrp4)
